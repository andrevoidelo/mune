  ...
  const renderFormattedText = (text: string) => {
    ...
    return parts.map((part, index) => {
      if (part.startsWith('**') && part.endsWith('**')) {
        return <strong key={index} className="text-white print:text-black font-bold">{part.slice(2, -2)}</strong>;
      }
              ...
              >
                ...
                {entry.type === 'NOTE' ? (
                  ...
                  <div className="mt-1">
                    ...
                    <p className="text-base text-slate-200 print:text-black font-serif print-serif leading-relaxed whitespace-pre-wrap">
                      {entry.result}
                    </p>
                  </div>
                ) : (
                  ...
                  <div className="mt-1">
                    <h3 className="font-bold text-slate-200 print:text-black text-sm">{entry.title}</h3>
                    <p className="text-lg text-white print:text-black font-medium mt-0.5 leading-snug">{entry.result}</p>
                  </div>
                )}
