...
import { User, Plus, Trash2, Edit2, X, Shield, Backpack, Minus, Dices, Activity, TrendingUp, TrendingDown, Target, PaintBucket, List, LayoutGrid, Sticker, Save, Search } from 'lucide-react';
...
const CharacterView: React.FC<CharacterViewProps> = ({ characters, setCharacters, addLog }) => {
  ...
  const confirmUseItem = () => {
    ...
    const getLogDetails = (detail?: string, total?: number) => {
      let prefix = '';
      if (detail && total !== undefined) {
          prefix = `${detail} ➔ **${total}** `;
      }

      if (isPermanent) return `${prefix}(Item permanente)`;
      
      if (autoReduceOnUse && currentQty > 0) {
        return `${prefix}(Restam: ${currentQty - 1})`;
      }
      return `${prefix}(Consumível)`;
    };
    ...
    if (diceNotation && diceNotation.trim() !== '') {
        ...
        // Extract commentary (text after the last dice formula term)
        const diceRegex = /([0-9dD+\-khlKHL!xXrRminax<>=]+)/gi;
        let lastMatchIndex = 0;
        let match;
        while ((match = diceRegex.exec(diceNotation)) !== null) {
            lastMatchIndex = diceRegex.lastIndex;
        }
        const commentary = diceNotation.substring(lastMatchIndex);

        setTimeout(() => {
           play('DICE_RESULT');
        }, 600);

        const resultWithCommentary = `**${diceResult.total}**${commentary}`;
  ...
  // --- Renderers ---
  // ... (All renderer functions: renderPreRollModal, renderRollResultModal, renderItemRollResultModal, renderUseItemModal, renderList, renderDetail, renderForm)
  // Need to include them all here. Since I am writing the file content, I will include them.
  // To save context window, I assume I have them from the previous read. I will write them now.
  
  // Actually, I can reuse the previous read content. 
  // I will write the full file content including all render functions.
  
  // NOTE: I'll include the full content in the write_file call.
  // ... (Full content construction) ...
        ...
        <div className="max-w-7xl mx-auto w-full pb-24">
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                ...
                <div 
                    ...
                    className={`aspect-[3/4] rounded-xl border relative shadow-lg transition-all active:scale-95 cursor-pointer flex flex-col group ${
                        isEditMode 
                        ? 'animate-pulse border-dashed border-primary/50 bg-card cursor-alias' 
                        : 'bg-card border-border hover:border-primary/50'
                    }`}
                >
                    <div className="flex-1 bg-app relative min-h-0 rounded-t-xl overflow-hidden">
                    {char.imageUrl ? (
                    ...
                    <div className="p-3 bg-card border-t border-border/50 rounded-b-xl">
                    ...
                    {isEditMode && (
                        <div 
                            onClick={(e) => { e.stopPropagation(); play('CLICK'); handleDelete(char.id); }}
                            className="absolute -top-2 -right-2 w-6 h-6 bg-error text-white rounded-full flex items-center justify-center shadow-lg z-20 hover:scale-110 transition-transform"
                        >
                            <Trash2 size={14} />
                        </div>
                    )}
          ...
          <div className="max-w-3xl mx-auto w-full min-h-full">
            ...
            <div className="pt-2">
              ...
              <div className="space-y-3">
                            ...
                            <div className="flex items-center gap-2">
                                <div className="flex-1">
                                  ...
                                  <input 
                                    ...
                                    className={`w-full rounded p-2 font-mono text-center outline-none border ${getInputStyle(attr.color).replace('bg-transparent', 'bg-black/10')}`}
                                  />
                                </div>
                                <div className="flex-1">
                                  <label className={`text-[9px] uppercase font-bold block mb-1 ${isDiceEmpty ? 'text-error' : getLabelStyle(attr.color)}`}>
                                    Dado {isDiceEmpty && '*'}
                                  </label>
                                  <input 
                                    type="text" 
                                    ...
                                    className={`w-full rounded p-2 font-mono text-center outline-none border ${isDiceEmpty ? 'border-error' : getInputStyle(attr.color).replace('bg-transparent', 'bg-black/10')}`}
                                    placeholder="d20"
                                  />
                                </div>
                            </div>
                            ...
                            <div className="flex items-center gap-2">
                                <div className="flex-1">
                                  ...
                                  <select 
                                    ...
                                    className={`w-full rounded p-2 text-xs outline-none border ${getInputStyle(attr.color).replace('bg-transparent', 'bg-black/10')}`}
                                  >
                                    <option value="UNDER" className="text-black">Menor (Skill)</option>
                                    <option value="OVER" className="text-black">Maior (Save)</option>
                                    <option value="TARGET" className="text-black">Alvo (DC)</option>
                                ...
                                {attr.rollType === 'TARGET' && (
                                    <div className="flex-1">
                                      ...
                                      <input 
                                        type="number" 
                                        value={attr.modifier || 0}
                                        onFocus={(e) => e.target.select()}
                                        onChange={(e) => updateAttribute(attr.id, 'modifier', parseInt(e.target.value) || 0)}
                                        className={`w-full rounded p-2 font-mono text-center outline-none border ${getInputStyle(attr.color).replace('bg-transparent', 'bg-black/10')}`}
                                        placeholder="+0"
                                      />
                                    </div>
                ...
                <p className="text-[10px] text-txt-dim mt-2">Dica: Use notações como "2d6+3" ou "1d20".</p>
              </div>
            </div>
            ...
            <div className="pt-2">
              ...
              <div className="space-y-3">
                ...
                >
                  <SortableContext
                    items={formData.inventory.map((item) => (typeof item === 'string' ? item : item.id))}
                    strategy={verticalListSortingStrategy}
                  >
                    {formData.inventory.map((item, idx) => {
                        if (typeof item === 'string') return null; // Should be migrated
                        const itemId = item.id;
                        return (
                        <SortableItem key={itemId} id={itemId}>
                            <div className="p-3 bg-card border border-border rounded-lg flex flex-col gap-3">
                                <div className="flex gap-2 items-start">
                                    <button
                                        type="button"
                                        onClick={(e) => { e.stopPropagation(); play('CLICK'); setEditingIconForItem(item.id); }}
                                        className={`p-2 rounded-lg border flex-none w-10 h-10 flex items-center justify-center transition-colors ${item.icon ? 'bg-card border-primary text-primary' : 'bg-app border-border text-txt-muted hover:text-txt-main'}`}
                                    >
                                        {item.icon ? (
                                            <div 
                                                className="w-6 h-6"
                                                style={{
                                                    backgroundColor: item.iconColor || 'currentColor',
                                    ...
                                    <div className="flex-1">
                                        <input 
                                            value={item.name}
                                            onChange={(e) => updateInventoryItemForm(itemId, { name: e.target.value })}
                                            className="w-full bg-transparent border-b border-border font-bold text-sm outline-none pb-1 text-txt-main focus:border-primary"
                                            placeholder="Nome do Item"
                                        />
                                    </div>
                                    <button onClick={() => { play('CLICK'); removeItem(itemId); }} className="text-txt-muted hover:text-error p-1">
                                        <Trash2 size={16} />
                                    </button>
                                </div>
                                ...
                                <div className="flex gap-3 items-center">
                                    <div className="flex items-center gap-2">
                                        <span className="text-[10px] uppercase font-bold text-txt-dim">Qtd</span>
                                        <div className="flex items-center bg-app rounded border border-border">
                                            <button 
                                                type="button"
                                                onClick={() => updateInventoryItemForm(itemId, { quantity: Math.max(0, item.quantity - 1) })}
                                                className="p-1 hover:bg-card-hover text-txt-muted"
                                            >
                                                <Minus size={12} />
                                            </button>
                                            <span className="px-2 font-mono text-xs font-bold text-txt-main">{item.quantity}</span>
                                            <button 
                                                type="button"
                                                onClick={() => updateInventoryItemForm(itemId, { quantity: item.quantity + 1 })}
                                                className="p-1 hover:bg-card-hover text-txt-muted"
                                            >
                                                <Plus size={12} />
                                            </button>
                                    ...
                                    <label className="flex items-center gap-2 text-[10px] uppercase font-bold text-txt-dim cursor-pointer ml-auto">
                                        <input 
                                            type="checkbox"
                                            checked={item.isPermanent}
                                            onChange={(e) => updateInventoryItemForm(itemId, { isPermanent: e.target.checked })}
                                ...
                                <div>
                                    <label className="text-[10px] uppercase font-bold text-txt-dim block mb-1">Dado de Dano/Efeito (Opcional)</label>
                                    <input 
                                        value={item.dice || ''}
                                        onChange={(e) => updateInventoryItemForm(itemId, { dice: e.target.value })}
                                        className="w-full bg-app border border-border rounded p-2 text-xs font-mono outline-none focus:border-primary text-txt-main"
            ...
            onSelect={(color) => {
                ...
                if (type === 'attribute') {
                    setFormData(prev => prev ? ({
                        ...prev,
                        attributes: prev.attributes.map(a => a.id === id ? { ...a, color } : a)
                    }) : null);
                } else if (type === 'resource') {
                    setFormData(prev => prev ? ({
                        ...prev,
                        resources: prev.resources.map(r => r.id === id ? { ...r, color } : r)
                    }) : null);
                }
            ...
            >
                ...
                >
                    <div className="flex-1 min-h-0 overflow-hidden">
                        <IconPicker 
                            selectedIcon={formData.inventory.find((i: any) => i.id === editingIconForItem)?.icon}
                            selectedColor={formData.inventory.find((i: any) => i.id === editingIconForItem)?.iconColor}
                            onSelect={(newIcon, newColor) => {
                                updateInventoryItemForm(editingIconForItem, { icon: newIcon, iconColor: newColor });
                            }}
    ...
    <>
      ...
      {tempImage && (
        <ImageEditorModal
          ...
          onSave={(croppedImage) => {
            setFormData(prev => prev ? ({ ...prev, imageUrl: croppedImage }) : null);
            setTempImage(null);
          }}
          aspectRatio={3/4}
        />
      ...
      <ConfirmDeleteModal 
        ...
        description={
          <>
            Tem certeza que deseja excluir <strong>{charToDelete?.name}</strong>?
            <br/>
            <span className="text-xs text-txt-dim mt-1 block">Esta ação não pode ser desfeita.</span>
          </>
        }
        onConfirm={confirmDelete}
        onCancel={() => { play('CLICK'); setCharToDelete(null); }}
      />
    </>
  );
};
