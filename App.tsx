...
const AppContent: React.FC = () => {
  ...
  useEffect(() => {
    if (isPaperMode) {
      ...
      document.querySelector('meta[name="theme-color"]')?.setAttribute('content', '#fcfbf7');
    } else {
  ...
  useEffect(() => {
    ...
    const savedTheme = localStorage.getItem('mune_theme');
    if (savedTheme) setIsPaperMode(savedTheme === 'paper');
  ...
  useEffect(() => {
    localStorage.setItem('mune_theme', isPaperMode ? 'paper' : 'dark');
  }, [isPaperMode]);
          ...
          adventures.map(adv => (
            ...
            >
              <div className="flex justify-between items-start mb-2">
                <div>
                  <h3 className="text-lg font-bold text-white group-hover:text-amber-500 transition-colors line-clamp-1">{adv.name}</h3>
      ...
      {showHelp && (
        ...
        >
           ...
           >
              ...
              <h3 className="text-xl font-bold text-white mb-2">{helpContent.title}</h3>
          ...
          <div className="bg-slate-800 border border-slate-700 rounded-xl p-6 max-w-sm w-full shadow-2xl relative animate-in zoom-in-95 duration-200">
            ...
            <div className="bg-slate-900/50 p-3 rounded-lg mb-4 border border-slate-800">
               ...
               <ul className="text-xs text-slate-400 list-disc list-inside space-y-1">
                 <li><strong className="text-white">{pendingImportData.adventures.length}</strong> Aventuras</li>
                 <li>
...
const NavButton: React.FC<{
  ...
  >
    ...
    {badge !== undefined && badge > 0 && (
      <span className="absolute top-1 right-1 sm:right-4 w-4 h-4 bg-red-600 text-white text-[9px] flex items-center justify-center rounded-full shadow-sm border border-slate-900">
        {badge > 99 ? '99+' : badge}
      </span>
    )}
  </button>
